...
case class Attack[Att: CanFight, Def: CanFight](attacker: Att, defender: Def, hitPoints: HitPoints, distance: Meters) {
  ...
  def damage: Def = {
    if ((attacker != this) && (attacker.range.canHit(distance)) && attacker.faction != defender.faction) {
      if (hitPoints.lessThanZero) defender else {
        val newHitPoints = this.defender.hitPoints - realDamage
        println(s"Hitpoints $hitPoints, $realDamage, $newHitPoints")
